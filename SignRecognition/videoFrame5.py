...
while success:
   ...
   success,image = vidcap.read()
   cv2.imshow("Webcam recording", image)
   print('Read a new frame: ', success, ' ', count)
   count += 1
   if count > 10: #Here because idk why ASL video never ends...
     success = False
   comp = False
   cv2.waitKey(2000)
...
if success == False:
  ...
  export = glob.glob("\Users\justi\Downloads\Sign-Language-Translator-master\SignRecognition\DATA\EXPORT\*.png")
  for pegs in export:
...
def compare_images(imageA, imageB, pic1, pic2, title):
    ...
    #2D array to hold all picture in D base (image Bs)

    if(m < 6000 and s> 0.75):
        # setup the figure
